...
struct scaled_result {
	...
	vector<double> s_scaled;			// Exciton concentration
...
struct scaling_params {
	...
	double B_c;		// Scaling of bimolecular rate [m3 s-1]
	double T_c;		// Scaling of trimolecular recombination rate [m6 s-1]
	double E_c;		// Scaling of energy [J]
	double A_c;		// Prefactor for dissociation [-]
	double b_c;		// Prefactor for bessel in dissociation [-]
};
...
struct scaled_numerical_params {
	...
	vector<double> u_s_scaled;				// nonuniform exciton mobility
...
struct scaled_layer_params {
	...
	double u_s_scaled;			// Mobility of excitons
	double E_b_scaled;			// Exciton binding energy
	double y_ns_scaled;			// Exciton-electron annihilation [m3 s^(-1)]
	double y_ps_scaled;			// Exciton-hole annihilation [m3 s^(-1)]	
	double t_s_scaled;			// Exciton lifetime [s]	
	...
	double E_t_scaled;			// Energy level of trapped state
...
struct scaled_boundaries {
	...
	double concentration_s_top_scaled;		// Exciton concentration - top
	double concentration_s_bottom_scaled;	// Exciton concentration - bottom
