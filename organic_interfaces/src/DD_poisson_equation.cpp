	...
	for (int i = 1; i <= N; i++)
	{
		...
		if (i > 1 && i < N)
		{
			...
			Vit_b.push_back( -0.5*(hi_scaled[i] + hi_scaled[i - 1]) * (
				p_it_scaled[i]
				- n_it_scaled[i]
				+ layer_params_scaled->N_D_scaled 
				- layer_params_scaled->N_A_scaled
				));
		}
...
void Drift_diffusion::generalized_potential(void)
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		tmp_phi_n_scaled.push_back(-(layer_params_ref_scaled->E_c_scaled - layer_params_scaled->E_c_scaled 
								- log(layer_params_ref_scaled->N_c_scaled / layer_params_scaled->N_c_scaled)));
		
		tmp_phi_p_scaled.push_back(-(layer_params_ref_scaled->E_v_scaled - layer_params_scaled->E_v_scaled 
								+ log(layer_params_ref_scaled->N_v_scaled / layer_params_scaled->N_v_scaled)));
