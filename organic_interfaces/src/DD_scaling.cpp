...
void Drift_diffusion::layer_params_scaling(unscaled_layer_params * layer_params_unscaled)
{
	...
	tmp_layer_params_scaled.u_s_scaled = mobility_scaling(layer_params_unscaled->u_s, &params_c);
	tmp_layer_params_scaled.E_b_scaled = energy_scaling(layer_params_unscaled->E_b, &params_c);
	tmp_layer_params_scaled.y_ns_scaled = bimolecular_scaling(layer_params_unscaled->y_ns, &params_c);
	tmp_layer_params_scaled.y_ps_scaled = bimolecular_scaling(layer_params_unscaled->y_ps, &params_c);
	tmp_layer_params_scaled.t_s_scaled = time_scaling(layer_params_unscaled->t_s, &params_c);
...
scaling_params c_parameters(double T)
{
	...
	parameters.A_c = 48*pow(C_PI*parameters.N, 2)*pow(parameters.x_c,6);
	parameters.b_c = pow(C_q,2) / (8*C_PI*C_E_0*parameters.Er_max*parameters.x_c*C_k_B*T);
