...
{
	double tmp_total_current, tmp_average_electron, tmp_average_holes, tmp_average_anion, tmp_average_displacement;
	...
	vector<double> anion_current_density = result_it1_scaled.J_a_scaled;
	//vector<double> cation_current_density = result_it1_scaled.J_c_scaled;
	...
	tmp_average_anion = average_vector(anion_current_density);
	//tmp_average_cation = average_vector(cation_current_density);
	tmp_average_displacement = average_vector(dipslacement_current_density);

	// Calculate total current density as a sum of J_n + J_p + J_NA + J_NC + J_displ
	tmp_total_current = tmp_average_electron + tmp_average_holes + tmp_average_anion + tmp_average_displacement;
