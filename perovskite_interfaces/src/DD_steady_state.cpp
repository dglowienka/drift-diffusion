...
double Drift_diffusion::steady_current(void)
{
	...
	vector<double> J_a_t1 = result_it1_scaled.J_a_scaled;
	vector<double> J_a_t = result_it_scaled.J_a_scaled;
	//vector<double> J_c_t1 = result_it1_scaled.J_c_scaled;
	//vector<double> J_c_t = result_it_scaled.J_c_scaled;

	// In steady current we do not include displacement current. As it is 0 when steady state takes place
	double tmp_average_J_tot_t1_scaled = average_vector(Jn_t1) + average_vector(Jp_t1) + average_vector(J_a_t1);
	double tmp_average_J_tot_t_scaled = average_vector(Jn_t) + average_vector(Jp_t) + average_vector(J_a_t);
