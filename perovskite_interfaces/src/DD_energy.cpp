...
void Drift_diffusion::energy(void)
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		tmp_Ec_scaled.push_back(current_layer_params_scaled->E_c_scaled - V_it1_scaled[i]);	
		tmp_Ev_scaled.push_back(current_layer_params_scaled->E_v_scaled - V_it1_scaled[i]);

		tmp_Efn_scaled.push_back(tmp_Ec_scaled[i] + log(n_it1_scaled[i] / current_layer_params_scaled->N_c_scaled));
		tmp_Efp_scaled.push_back(tmp_Ev_scaled[i] - log(p_it1_scaled[i] / current_layer_params_scaled->N_v_scaled));		
	}
