...
void Drift_diffusion::set_mesh(void) 
{
	...
	grid_scaled.u_a_scaled.push_back(tmp_layer->u_a_scaled);	
	grid_scaled.u_c_scaled.push_back(tmp_layer->u_c_scaled);
	...
	for (list<scaled_layer_params>::iterator it=layers_params_scaled.stack_params.begin(); it != layers_params_scaled.stack_params.end(); ++it)
	{
		...
		set_mesh_variable(&grid_scaled.u_a_scaled, it->u_a_scaled, it->N_points);	// Anion mobility
		set_mesh_variable(&grid_scaled.u_c_scaled, it->u_c_scaled, it->N_points);	// Cation mobility
	}
