...
private:
	...
	// Temporal parameters - might be used in every function for forward iteration within layers
	scaled_layer_params * current_layer_params_scaled; // Current layer parameters
	int current_i_position; // Current position in the
	...
	// Calculate current of anions density in t+1 time
	void anion_current_density(void);
	// Calculate current of cations density in t+1 time
	void cation_current_density(void);
	...
	double R_Mn(void);
	// Conduction band SRH for trap distribution
	double R_M_conduction(double E);
	// Donor region for dynamic traps
	double R_Mn_D(void);
	// Acceptor region for dynamic traps
	double R_Mn_A(void);
	// MONOMOLECULAR RECOMBINATION FOR ELECTRONS (SRH type)
	double R_Mp(void);
	// Valence band SRH for trap distribution
	double R_M_valence(double E);
	// Donor region for dynamic traps
	double R_Mp_D(void);
	// Acceptor region for dynamic traps
	double R_Mp_A(void);
	...
	// Integrate trapezoidal function 
	double integrate(double(Drift_diffusion::*func)(double), double a, double b, int n);
